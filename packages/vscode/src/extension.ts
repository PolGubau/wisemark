...
export function activate(context: vscode.ExtensionContext) {
	...
	const scanComments = async () => {
		...
		const result = await listAndFilter(
			{ cwd: wsPath, path: "." },
	...
	context.subscriptions.push(
		...
		vscode.commands.registerCommand("wisemark.scan", scanComments),
		vscode.commands.registerCommand("wisemark.refresh", scanAndNotify),
		vscode.commands.registerCommand("wisemark.toggleGrouping", () =>
			toggleGrouping(commentProvider),
		),
		vscode.commands.registerCommand("wisemark.insertTodoComment", () =>
			insertNoteComment("todo"),
		),
		vscode.commands.registerCommand("wisemark.insertNoteComment", () =>
			insertNoteComment("note"),
		),
		vscode.commands.registerCommand("wisemark.insertFixmeComment", () =>
			insertNoteComment("fixme"),
		),
		vscode.commands.registerCommand("wisemark.insertQuestionComment", () =>
			insertNoteComment("question"),
		),
		vscode.commands.registerCommand("wisemark.insertRefactorComment", () =>
			insertNoteComment("refactor"),
		),
	);
}
...
const COMMENT_TAGS: CommentTag[] = [
	...
	{
		label: "question",
		detail: "Question or clarification needed",
		severity: "low",
	},
